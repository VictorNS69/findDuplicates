...
def main():
    userData = parse()
    try:
        target = FileInfo.FileInfo(userData.file)
    except OSError as e:
        _ERR("Error reading file %s: %s" % (userData.file, e))
        sys.exit(1)
        
    files = FileSet.FileSet(userData.path, userData.recursive)
    try:
    ...
    candidates = [FileInfo.FileInfo(f) for f in files.search_by_size(target.size)]
